...
import { omit, includes, pick, isEmpty } from 'lodash';
...
export function getErrorMsgsInEnglish(validation, errorMsgs) {
  const { messages, messagesEn } = validation;
  const enErrorMsgs = {};
  for (const field in errorMsgs) {
    if (errorMsgs.hasOwnProperty(field)) {
      for (const errorName in messages[field]) {
        if (messages[field].hasOwnProperty(errorName)) {
          if (messages[field][errorName] === errorMsgs[field][0]) {
            enErrorMsgs[field] = [messagesEn[field][errorName]];
            break;
          }
        }
      }
  ...
  return isEmpty(enErrorMsgs) ? undefined : enErrorMsgs;
}
