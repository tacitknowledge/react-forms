...
export class FormGroup extends Component {
  ...
  onInvalidate(e) {
    ...
    if (!isValid) {
      eventDetails.errorMsgs = {
        [e.currentTarget.getAttribute('name')]: errorMsgs
      };
      let event;
      try {
        event = new CustomEvent(FORM_SUBMIT_LISTENER, { detail: eventDetails });
      document.dispatchEvent(event);
      } catch(error) {
        event = document.createEvent('CustomEvent');
        event.initCustomEvent(FORM_SUBMIT_LISTENER, false, false, eventDetails);
        document.dispatchEvent(event);
      }
      setSingleValidity(
        formName,
      ...
      document.dispatchEvent(new CustomEvent(FORM_FIELD_LISTENER, { detail: eventDetails }));
      let event;
      try {
        event = new CustomEvent(FORM_FIELD_LISTENER, { detail: eventDetails });
        document.dispatchEvent(event);
      } catch(error) {
        event = document.createEvent('CustomEvent');
        event.initCustomEvent(FORM_FIELD_LISTENER, false, false, eventDetails);
        document.dispatchEvent(event);
      }
    }
  }
