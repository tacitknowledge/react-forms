...
import { ERROR_INPUT_CLASS_NAME, HAS_VALUE_CLASS_NAME, FORM_FIELD_LISTENER } from './formConstants';
...
function getErrorMsg(name, errors) {
  ...
  return (errors[name] || [])[firstArrayItem];
}
  ...
  return {
    ...
    onBlur: e => {
      ...
      if (hasTheValueChanged) {
        const event = new CustomEvent(FORM_FIELD_LISTENER, { detail: {
          custom_info: formGroupProps,
