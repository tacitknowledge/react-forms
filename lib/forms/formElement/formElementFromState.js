...
export class FormElementFromState {
  ...
  checkbox() {
    return this.stateValues[this.name] ? { checked: this.stateValues[this.name] } : null;
  }

  radio() {
    return this.stateValues[this.name] ? { checked: this.stateValues[this.name] === this.value } : null;
  }
